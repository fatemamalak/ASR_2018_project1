...
def _preprocess_data(args):
    ...
    for full_wav in full_wavs:
        ...
        mfcc_feats = compute_mfcc(wav_file[durations[0][0]:durations[0][1]], n_delta)

        for i in range(len(mfcc_feats)):
                labels.append(trans[0])
        for index, chunk in enumerate(durations[1:]):
            mfcc_feat = compute_mfcc(wav_file[chunk[0]:chunk[1]], n_delta)
    ...
    timit_df.to_hdf("./features/mfcc_delta_delta/Test/timit.hdf", "timit")
